...
const asyncValidate = (values, dispatch) => {
  return dispatch(validateUserFields(values)).then((result) => {
      //Note: Error's "data" is in result.payload.response.data
      // success's "data" is in result.payload.data
      if (result.payload.data == null) { //1st onblur
        return;
      }

      let {data, status} = result.payload;

      //if any one of the fields exist, then there is a field error
  ...
  return dispatch(signUpUser(values))
    .then((result) => {
      ...
      if (result.payload.data == null) {
        dispatch(signUpUserFailure(result.payload.data));
